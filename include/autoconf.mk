...
CONFIG_BOARDDIR="board/myirtech/myd_c335x"
CONFIG_OMAP3_SPI=y
CONFIG_SPL_NAND_AM33XX_BCH=y
CONFIG_PHYLIB=y
CONFIG_CMDLINE_EDITING=y
CONFIG_ENV_OFFSET_REDUND=0x001e0000
CONFIG_BOOTCOMMAND="run findfdt; run envboot;usb dev 0;run usbboot;mmc dev 0;if mmc rescan; then setenv mmcdev 0;setenv devtype mmc; setenv devnum 0;setenv bootpart 0:2; run findfdt; run mmcboot;fi; mmc dev 1; if mmc rescan; then setenv mmcdev 1;setenv devtype mmc;setenv devnum 1;setenv bootpart 1:2; run findfdt; run mmcboot;fi; run nandboot; "
CONFIG_TIMESTAMP=y
...
CONFIG_EXTRA_ENV_SETTINGS="DEFAULT_LINUX_BOOT_ENV DEFAULT_MMC_TI_ARGS "boot_fdt=try0bootpart=0:20bootdir=/boot0bootfile=zImage0fdtfile=undefined0console=ttyO0,115200n80partitions=uuid_disk=${uuid_gpt_disk};name=rootfs,start=2MiB,size=-,uuid=${uuid_gpt_rootfs}0optargs=0usbroot=/dev/sda2 rw0usbrootfstype=ext4 rootwait0usbdev=00ramroot=/dev/ram0 rw0ramrootfstype=ext20spiroot=/dev/mtdblock4 rw0spirootfstype=jffs20spisrcaddr=0xe00000spiimgsize=0x3620000spibusno=00spiargs=setenv bootargs console=${console} ${optargs} root=${spiroot} rootfstype=${spirootfstype}0usbargs=setenv bootargs console=${console} ${optargs} root=${usbroot} rootfstype=${usbrootfstype}0ramargs=setenv bootargs console=${console} ${optargs} root=${ramroot} rootfstype=${ramrootfstype}0loadramdisk=load ${devtype} ${devnum} ${rdaddr} ramdisk.gz0loadimage=load ${devtype} ${bootpart} ${loadaddr} ${bootdir}/${bootfile}0loadfdt=load ${devtype} ${bootpart} ${fdtaddr} ${bootdir}/${fdtfile}0loadbootenv=load ${devtype} ${devnum} ${loadaddr} ${bootenvfile}0importbootenv=echo Importing environment from ${devtype} ...; env import -t ${loadaddr} ${filesize}0mmcloados=run args_mmc; if test ${boot_fdt} = yes || test ${boot_fdt} = try; then if run loadfdt; then bootz ${loadaddr} - ${fdtaddr}; else if test ${boot_fdt} = try; then bootz; else echo WARN: Cannot load the DT; fi; fi; else bootz; fi;0mmcboot=mmc dev ${mmcdev}; setenv devnum ${mmcdev}; setenv devtype mmc; if mmc rescan; then echo SD/MMC found on device ${mmcdev};run envboot; if run loadimage; then run mmcloados;fi;fi;0spiboot=echo Booting from spi ...; run spiargs; sf probe ${spibusno}:0; sf read ${loadaddr} ${spisrcaddr} ${spiimgsize}; bootz ${loadaddr}0ramboot=echo Booting from ramdisk ...; run ramargs; bootz ${loadaddr} ${rdaddr} ${fdtaddr}0usbboot=setenv devnum ${usbdev}; setenv devtype usb; usb start ${usbdev}; if usb dev ${usbdev}; then if run loadbootenv; then echo Loaded environment from ${bootenvfile};run importbootenv;fi;if test -n $uenvcmd; then echo Running uenvcmd ...;run uenvcmd;fi;if run loadimage; then run loadfdt; echo Booting from usb ${usbdev}...; run usbargs;bootz ${loadaddr} - ${fdtaddr}; fi;fi0fi;usb stop ${usbdev};0findfdt=if test $board_name = A335BONE; then setenv fdtfile am335x-bone.dtb; fi; if test $board_name = A335BNLT; then setenv fdtfile am335x-boneblack.dtb; fi; if test $board_name = BBG1; then setenv fdtfile am335x-bonegreen.dtb; fi; if test $board_name = A33515BB; then setenv fdtfile am335x-evm.dtb; fi; if test $board_name = A335X_SK; then setenv fdtfile am335x-evmsk.dtb; fi; if test $board_name = A335_ICE; then setenv fdtfile am335x-icev2.dtb; fi; if test $board_name = myd_c335x; then setenv fdtfile myd_c335x.dtb; fi; if test $fdtfile = undefined; then echo WARNING: Could not determine device tree to use; fi; 0init_console=if test $board_name = A335_ICE; then setenv console ttyO3,115200n8;else setenv console ttyO0,115200n8;fi;0" NANDARGS NETARGS"
CONFIG_BOOTP_PXE=y
...
CONFIG_MACH_TYPE="MACH_TYPE_MYD_C335X"
